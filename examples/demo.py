...
def main():
    ...
    cl_strategy.config = LNSConfig(seed=123, relax_rate=40, init_time_limit=2, time_limit=None, max_steps=500)

    # constrained LNS using clingo with assumptions and random relaxation
    con_strategy = DefaultStrategy()
    con_strategy.config = LNSConfig(seed=123, relax_rate=40, init_time_limit=2, time_limit=20, max_steps=500, constrained=True)
    ...
    decl_custom.config = LNSConfig(seed=123, relax_rate=20, init_time_limit=2, max_steps=5, declarative=True)

    lns = LNS(["examples/golf_demo.lp"], cl_strategy)

    # Example using heulingo strategy
    heuristic_strategy = Heulingo()
    heuristic_strategy.config = HeulingoConfig(seed=123, init_time_limit=2, lns_time_limit=2, max_steps=500, clingo_args="-c n=40")
